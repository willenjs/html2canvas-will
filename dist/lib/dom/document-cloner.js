        ...
        declaration.content.forEach(function (token) {
            if (token.type === tokenizer_1.TokenType.STRING_TOKEN) {
                ...
                if (token.name === 'attr') {
                    ...
                    var _a = token.values.filter(parser_1.nonFunctionArgSeperator), counter = _a[0], counterStyle = _a[1];
                    ...
                    var _b = token.values.filter(parser_1.nonFunctionArgSeperator), counter = _b[0], delim = _b[1], counterStyle = _b[2];
                    if (counter && parser_1.isIdentToken(counter)) {
